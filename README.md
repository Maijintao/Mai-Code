...
# 或用 CLI 直接跑任务
uv run Mai run --goal "帮我写一个待办清单工具"
```
...
```
src/mai_code/
  cli/           # CLI 客户端（Mai / mai-core / mai-tui 入口）
  core/
...
配置优先级（低 → 高）：**内建默认值 → `~/.mai/config.toml` → `.env` → 系统环境变量**。

常用环境变量（详见 `.env.example`）：

- `MAI_HOST` / `MAI_PORT`：daemon 监听地址
- `MAI_LOG_LEVEL` / `MAI_LOG_FILE` / `MAI_LOG_FORMAT`：日志
- `MAI_LLM_DEFAULT_MODEL`：默认模型
- `MAI_MAX_STEPS`：Agent Loop 最大执行步数（防止死循环）
- `MAI_PERMISSION_TIMEOUT_S`：权限审批超时
