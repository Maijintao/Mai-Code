...
import mai_code
from mai_code.core.bus.commands import PongResult
from mai_code.core.bus.envelope import JsonRpcError, JsonRpcSuccess
from mai_code.core.config import MaiConfig


# 同步入口：运行 ping 协程，连接失败时打印错误并退出
def cmd_ping(config: MaiConfig) -> None:
    try:
...
async def _ping(config: MaiConfig) -> None:
    t0 = time.monotonic()
    ...
    req = {
        ...
        "id": "cli-1",
        "method": "core.ping",
        "params": {"client": f"cli/{mai_code.__version__}"},
    }
